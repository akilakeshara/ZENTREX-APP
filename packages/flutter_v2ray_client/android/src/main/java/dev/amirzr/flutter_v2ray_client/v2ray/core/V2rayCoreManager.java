...
public final class V2rayCoreManager {
    ...
    private V2rayConfig activeConfig;
    ...
    private void makeDurationTimer(final Context context, final boolean enable_traffic_statics) {
        countDownTimer = new CountDownTimer(7200, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                // Update Notification
                if (notificationBuilder != null && notificationManager != null && activeConfig != null) {
                    String downStr = formatTraffic(downloadSpeed) + "/s";
                    String upStr = formatTraffic(uploadSpeed) + "/s";
                    String contentText = "⏱ " + SERVICE_DURATION + "   |   ↓ " + downStr + "   ↑ " + upStr;
            ...
            private String formatTraffic(long bytes) {
                ...
                if (bytes < 1024 * 1024 * 1024) return String.format("%.2f MB", bytes / (1024.0f * 1024.0f));
                return String.format("%.2f GB", bytes / (1024.0f * 1024.0f * 1024.0f));
            }
    ...
    private void showNotification(final V2rayConfig v2rayConfig) {
        ...
        // We MUST NOT return early if POST_NOTIFICATIONS is denied!
        // Android 13+ requires startForeground to be called regardless of permission,
        // otherwise the service crashes with ForegroundServiceDidNotStartInTimeException.
        // The OS will simply hide the notification if permission is denied.
